...
class TempBan extends Command {
    constructor(bot) {
        ...
        this.run = async (msg, args) => {
            ...
            const reason = `[${msg.author.username.replace(/[^\x00-\x7F]/g, "")}#${msg.author.discriminator}] - ${msg.content.split(' ').slice(3).join(' ') || `[${msg.author.username.replace(/[^\x00-\x7F]/g, "")}#${msg.author.discriminator}] - ban command issued (no reason given, banned for ${args[1]})`}, banned for ${args[1]}`;
